	\begin{tikzpicture}[x=13mm, y=12mm ,>=stealth',bend angle=45,auto]
		...
		\node[draw, fit={(0.1,1.50) (2.17,1.85)},
			inner sep=0pt,
		...
		% Estimating p^f
		\node[draw, fit={(4.2,1.0) (8.0,2.37)},
			inner sep=0pt,
			fill=mblue!20
			] (pf) {};
			%edge [<-] node [above] {$\boldsymbol{x}^k_j$} (rob);
		\node[draw, fit={(4.3,1.1) (7.9,1.5)},
			inner sep=0pt,
			fill=mblue!50,
			label=center:{Rolling horizon}
			] (rr) {};
		\node[draw, fit={(4.3,1.8) (7.9,2.2)},
			inner sep=0pt,
			fill=mblue!50,
		...
		\node[draw, fit={(0.0,2.9) (2.8,3.5)},
			...
			] (dm) {} edge[out=270,in=90,->] node [yshift=2.4em,xshift=0.5em,below right] {$\mathcal{D}_{j,k}$} (hm);
