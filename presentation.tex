...
\title[Planning \& scheduling with degradation]{Robust Planning and Scheduling
for Processes with Equipment Degradation \vspace{-10pt}}
...
\begin{frame}{A health model based on L\'evy processes}
    ...
    \begin{overlayarea}{\textwidth}{0.70\textheight}
        ...
        &
        && S_{j,t} \leq s_{j}^{max}
        &&& \forall t, j \in J\phantom{, \tilde{d}_{j,k} \in \mathcal{U}}\\
        &
        && S_{j,t} =
        \begin{cases}
            S_{j,t-1} +
            ...
            {K}}{x_{j,k,t}\cdot}}} D_{j{\color{imperialAlertText}{,k}}}},
            & \text{if } m_{j,t} = 0\\
        ...
        \begin{equation*}
            \begin{aligned}
                ...
                \begin{cases}
                    ...
                    {K}}{x_{j,k,t}\cdot}}}
                    D_{j{\color{imperialAlertText}{,k}}}, & \text{if } m_{j,t} = 0\\ s_{j}^{0}, & \text{otherwise}
                \end{cases}
                &&& \forall t, j \in J \phantom{, \tilde{d}_{j,k} \in
            \mathcal{U}.}\\
            \end{aligned}
        \end{equation*}
...
\begin{frame}[t]{Conclusion: preprint at
        \href{https://arxiv.org/abs/1810.09289}{\underline{arXiv:1810.09289}}
}
    \vspace{-10pt}
    \centering
...
%\begin{block}{Random variables can be replaced by uncertain parameters}
    ...
    && s_{j,t} =
    \begin{cases}
    s_{j,t-1} + \sum_{k \in {K}}{x_{j,k,t}\cdot \tilde{d}_{j,k}}, & \text{if } m_{j,t} = 0\\
    s_{j}^{0}, & \text{otherwise}
    \end{cases}
